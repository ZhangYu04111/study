...
def dashboard_summary():
    vehicles = list_vehicles()    # 获取车型列表 来源于SQLite数据库
    logs = list_recommendation_logs()    # 数据库中的日志
    leads = list_leads()    # 销售线索
    energy = Counter([v["energy_type"] for v in vehicles])    # 对车型中的能源类型计数
    type_counter = Counter([v["vehicle_type"] for v in vehicles])     # 车的类型计数
    hot_models = sorted(vehicles, key=lambda x: x["monthly_sales"], reverse=True)[:8]    # 根据月销量获取前八个车型

    budget_buckets = Counter()    # 计算不同预算的区间
    ...
    for lead in leads:
        ...
        for item in (lead.get("concerns") or "").split(","):  # 记录不同关注点的数量
            if item:
